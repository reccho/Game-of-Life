		...
		private void SetNeighbors() {
            for (int x = 0; x < col; x++) {
                for (int y = 0; y < row; y++) {
                    ...
                    int above = (y == 0) ? row - 1 : y - 1;			// Wrap over-reaching indices
                    int below = (y == row-1) ? 0 : y + 1;			//

                    Cells[x,y].Neighbors.Add(Cells[left, y]);		//Neighbor to the left
                    Cells[x,y].Neighbors.Add(Cells[right, y]);		//Neighbor to the right
