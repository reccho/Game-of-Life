...
namespace GoL
{
	public partial class Form1 : Form
	{
        ...
        private void Bttn_Clear_Click(object sender, EventArgs e) {
            ...
            board.Clear();  //Randomize w/ 0 live cells --> empty board
		...
		private void DrawBoard() {
            ...
            //label_Gen.Text = "Generation: " + board.Cells.Length.ToString();      //display number of cells
            label_Gen.Update();
        ...
        private void Reset(string preset) {
            ...
            if (outgrown) {
                Form_TooSmall note = new Form_TooSmall();
                note.ShowDialog();
                //Console.WriteLine("Board is too small for this pattern.");
			}
            generation = 0; //Reset generation counter
            DrawBoard();
        }

        private void Reset(bool randomized = true) {
            board = new Board(pictureBox1.Width, pictureBox1.Height, (int)UpDown_Size.Value);
            if (randomized) 
                board.Randomize();
            generation = 0;
